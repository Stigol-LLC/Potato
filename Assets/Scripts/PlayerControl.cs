...
public class PlayerControl : MonoBehaviour, ITouchable {
    #region Properties
    ...
    private float _lenghtMoveTouch = 0.1f;
    private float h;
    private Gun _gun;
    ...
    public bool TouchMove( Vector2 touchPoint ) {
        ...
        if ( ! vertical &&
             ( horizontalSlideLenght < -_lenghtMoveTouch ) ) {
            h = 1.0f;
        } else if ( ! vertical &&
                    horizontalSlideLenght > _lenghtMoveTouch ) {
            h = -1.0f;
        } else if ( verticalSlideLenght < -_lenghtMoveTouch && grounded ) {
            jump = true;
        } else if ( verticalSlideLenght > _lenghtMoveTouch ) {
            if ( _gun ) {
                _gun.Fire = true;
            }
        }
    ...
    private void Awake() {
        ...
        TouchProcessor.Instance.AddListener( this, 0 );
    ...
    private void FixedUpdate() {
        ...
        if ( Mathf.Abs( rigidbody2D.velocity.x ) > maxSpeed ) {
            ...
            rigidbody2D.velocity = new Vector2(
                    Mathf.Sign( rigidbody2D.velocity.x ) * maxSpeed,
